...
`;
          ...
          .then((response) => {
            ...
            for (let i = 0; i < response.data.length; i++) {
              ...
              const data = `
                ...
                <img class=" rounded bg-cover"  src="${product.image.url
  ...
  async getProductsWithLimitedOffers() {
    ...
    console.log({
      ...
      DateTime: DateTime.fromFormat(offerDate.innerText?.trim(), 'yyyy-mm-dd').toObject(),
    });

    this._makeCountDown(DateTime.fromFormat(offerDate.innerText?.trim(), 'yyyy-mm-dd').toJSDate());
